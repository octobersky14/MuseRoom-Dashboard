...
const useDirectSpeechRecognition = (): DirectSpeechRecognitionHook => {
      ...
      recognition.onresult = (event: SpeechRecognitionEvent) => {
        ...
        for (let i = event.resultIndex; i < event.results.length; i++) {
          ...
          if (result.isFinal) {
            ...
            console.log('Final transcript:', text, 'Confidence:', result[0].confidence);
          } else {
        ...
        if (finalTranscript) {
          ...
          if (interimTranscript.length > 3 && interimTranscript !== transcript) {
  ...
  const startListening = useCallback(async () => {
    ...
    try {
      ...
      setTimeout(() => {
        if (!isListening && recognitionAttemptsRef.current < maxRecognitionAttempts) {
          console.log(`Recognition didn't start, retrying (attempt ${recognitionAttemptsRef.current + 1}/${maxRecognitionAttempts})`);
  ...
  }, [browserSupportsSpeechRecognition, createRecognitionInstance, isListening, micPermission, requestMicrophonePermission]);
  ...
  const confirmTranscript = useCallback((): string => {
    ...
    const confirmedTranscript = editableTranscript || transcript || rawTranscriptRef.current;
