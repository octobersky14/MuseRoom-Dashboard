...
BUILD_DIR="$MCP_CLIENT_DIR/build"
...
if [ ! -f "$BUILD_DIR/index.js" ]; then
  print_error "Build failed: $BUILD_DIR/index.js does not exist"
  exit 1
fi
...
# Step 4: Create a package.json in the build directory
print_step "Creating package.json in build directory"
...
# Step 9: Copy the mcp-client-typescript build to the publish directory for direct access
print_step "Copying mcp-client-typescript to publish directory"
mkdir -p "$PROJECT_ROOT/dist/mcp-client-typescript"
cp -r "$BUILD_DIR"/* "$PROJECT_ROOT/dist/mcp-client-typescript/"
