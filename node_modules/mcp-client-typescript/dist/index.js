...
// 获取当前时间的工具
server.tool('get_current_time', // 工具名称
'获取当前时间', // 工具描述
{
    // 工具参数
...
async ({ timezone }) => {
    ...
    return {
        content: [
            { type: 'text', text: JSON.stringify({ currentTime }, null, 2) }
        ]
    };
});
// 时间转换工具
server.tool('convert_time', // 工具名称
'在时区之间转换时间', // 工具描述
{
    // 工具参数
...
async ({ source_timezone, time, target_timezone }) => {
    ...
    return {
        content: [
            { type: 'text', text: JSON.stringify({ convertedTime }, null, 2) }
        ]
    };
});
// 启动服务器
